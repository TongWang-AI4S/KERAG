...
在处理特定领域任务时， Agent 的表现往往依赖于它所能获取的背景知识。如果能为 Agent 提供一套组织严密、逻辑清晰的背景资料，它的表现大幅提升。以前，我们会复用pip或npm的模块，而现在，或许我们也可以让“知识”能够下载、安装与复用。KERAG 的目标就是实现这种“知识模块化”，让每个人整理的系统性经验都能成为可分发、可组装的资产，从而让 Agent 更好地应对各个领域的任务。

同时，面向高度结构化的领域知识，我们采取了将 Markdown 文件解析为具有逻辑层级的“知识树”的风格。这种结构允许 AI Agent 像人类翻阅教科书目录一样，通过层级导航来精准定位和检索知识，与此前的向量检索式RAG形成了互补。
